package sprint5.task11;

import java.io.BufferedWriter;
import java.io.IOException;
import java.io.OutputStreamWriter;

// Напишите функцию, которая будет выводить по неубыванию все ключи от L до R включительно в заданном бинарном дереве
// поиска.
...
public class Solution {
    ...
    public static void printRange(Node root, int left, int right, BufferedWriter writer) throws IOException {
        if (root.getValue() >= left && root.getLeft() != null) {
            printRange(root.getLeft(), left, right, writer);
        }

        if (root.getValue() >= left && root.getValue() <= right) {
            writer.write(root.getValue() + " ");
        }

        if (root.getValue() <= right && root.getRight() != null) {
            printRange(root.getRight(), left, right, writer);
        }
    }
