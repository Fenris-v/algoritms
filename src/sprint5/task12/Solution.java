...
// надо сделать просеивание вниз. Функция должна вернуть индекс, на котором элемент оказался после просеивания. Также
// необходимо изменить порядок элементов в переданном в функцию массиве.
//
...
public class Solution {
    public static int siftDown(int[] heap, int idx) {
        ...
        int nextIndex = right < heap.length && heap[left] < heap[right] ? right : left;
        if (heap[nextIndex] <= heap[idx]) {
            return idx;
        }
        ...
        return siftDown(heap, nextIndex);
    }
}
